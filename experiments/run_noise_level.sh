...
do
    ...
    do
        python -u -m experiments.run_simulation --method=${method} --device=${device} --sample=${sample_total} --batch_size=10 --path=${model_path} --data_path=${data_path} > "results/noise_${noise}_${method}.txt"
    done
