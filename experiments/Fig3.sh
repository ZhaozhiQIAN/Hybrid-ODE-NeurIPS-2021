...
python -u -m experiments.run_simulation --method=hybrid --device=${device} --sample=${sample} --path=${model_path} --batch_size=10 > "results/sample_${sample}_h.txt"
...
sample=310
model_path="model/model_sample_${sample}/"
python -u -m experiments.run_simulation --method=neural --device=${device} --sample=${sample} --path=${model_path} --batch_size=10 > "results/sample_${sample}_n.txt"
...
sample=310
model_path="model/model_sample_${sample}/"
python -u -m experiments.run_simulation --method=expert --device=${device} --sample=${sample} --path=${model_path} --batch_size=10 > "results/sample_${sample}_e.txt"
...
do
    ...
    do
        model_path="model/model_sample_${sample}/"
        python -u -m experiments.run_simulation --method=${m} --device=${device} --sample=${sample} --path=${model_path} --data_path=${data_path} --eval=y > "results/sample_${sample}_${m}.txt" &
    done
done
