...
do
    ...
    python -u run_simulation.py --method=neural --encoder_output_dim=$encoder_output_dim --device=${device} --batch_size=10 --sample=${sample_total} --data_path=${data_path} --path=${model_path} > "results/sample_${sample}_Z_${encoder_output_dim}.txt"
done


rm results/results_z.txt
