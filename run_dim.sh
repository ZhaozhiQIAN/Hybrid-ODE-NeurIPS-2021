...
method=expert
python -u run_simulation.py --method=${method} --device=${device} --sample=${sample_total} --path=${model_path} --data_path=${data_path} --data_config="dim${dim}" > "results/dim${dim}_${method}.txt" &
...
method=hybrid
python -u run_simulation.py --method=${method} --device=${device} --sample=${sample_total} --path=${model_path} --data_path=${data_path} --data_config="dim${dim}" --seed=4 > "results/dim${dim}_${method}.txt" 

method=neural
python -u run_simulation.py --method=${method} --device=${device} --sample=${sample_total} --path=${model_path} --data_path=${data_path} --data_config="dim${dim}" > "results/dim${dim}_${method}.txt" 

echo "Ensemble"
python -u run_simulation_residual.py --method=residual --device=${device} --sample=${sample_cali} --path=${model_path} --data_path=${data_path} --data_config="dim${dim}" --lr=0.001 > "results/dim${dim}_residual.txt" 
python -u run_simulation_ensemble.py --method=ensemble --device=${device} --sample=${sample_cali} --path=${model_path} --data_path=${data_path} --data_config="dim${dim}" > "results/dim${dim}_ensemble2.txt" 
