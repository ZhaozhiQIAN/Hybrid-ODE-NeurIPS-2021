...
def run(seed: int,
    ...
    for i in range(n_restart):
        encoder = model.EncoderPlanarLSTM(obs_dim + action_dim, int(obs_dim*encoder_latent_ratio), dg.latent_dim, num_flows, normalize=False, device=device)

        decoder = model.RocheExpertDecoder(obs_dim, encoder_output_dim, action_dim, t_max, step_size, roche=roche, method=ode_method, device=device)

        vi = model.VariationalInferenceFlow(encoder, decoder, prior_log_pdf=prior,
                                            elbo=elbo, mc_size=50)
...
if __name__ == '__main__':
    ...
    run(seed, elbo, device, eval_only, init_path, data_path, sample, data_config, roche_config, model_config, optim_config, eval_config)
