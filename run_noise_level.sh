...
do
    nohup python -u run_simulation.py --method=${method} --device=${device} --sample=${sample_total} --path=${model_path} --data_path=${data_path} > "results/noise_${noise}_${method}.txt" &
done

nohup python -u run_simulation_flow.py --method=hybrid --device=${device} --sample=${sample_total} --path=${model_path} --data_path=${data_path} > "results/noise_${noise}_flow.txt" &

# noise = 0.4 => seed = 1
nohup python -u run_simulation.py --seed=1 --method=hybrid --device=${device} --sample=${sample_total} --path=${model_path} --data_path=${data_path} > "results/noise_${noise}_${method}.txt" &

model_path="model/model_noise_${noise}/"
sample_cali=`expr ${sample} - 300`
nohup python -u run_simulation_ensemble.py --method=ensemble --device=${device} --sample=${sample_cali} --path=${model_path} --data_path=${data_path} > "results/noise_${noise}_ensemble2.txt" &
nohup python -u run_simulation_residual.py --method=residual --device=${device} --sample=${sample_cali} --path=${model_path} --data_path=${data_path} > "results/noise_${noise}_residual.txt" &
