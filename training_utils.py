...
def evaluate(model, data_generator, batch_size, t0, mc_itr=50, real=False):
    with torch.no_grad():
        ...
        for chunk in range(data_generator.test_size // batch_size):
            ...
            x_hat = x_hat[t0:, ...]
            total_rmse_z0.append(
                torch.sum((z0[:, : data_generator.expert_dim] - z0_hat[:, : data_generator.expert_dim]) ** 2, dim=1)
            )
...
def evaluate_horizon(model, data_generator, batch_size, t0, mc_itr=10, real=False):
    with torch.no_grad():
        ...
        d = {"rmse_x": rmse_x, "rmse_x_sd": rmse_x_sd, "cprs_x": cprs_x, "cprs_x_sd": cprs_x_sd}
        return d
...
def evaluate_ensemble_horizon(
    model_expert, model_ml, data_generator, batch_size, t0, mc_itr=10, weight_expert=1, weight_ml=1
):
    with torch.no_grad():
        # sample-level rmse
        total_rmse_x = list()
